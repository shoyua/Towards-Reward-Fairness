...
class BasicTrainer(object):
    ...
    def eval(self) -> Dict[str, Dict]:
        ...
        for eval_batch in (
            tqdm(eval_dataloader, desc="Computing eval metrics")
        ...
        for batch in self.train_iterator:
            ...
            with self.accelerator.accumulate(self.policy):
                ...
                if self.accelerator.sync_gradients:
                    # self.config.model.max_grad_norm=1
                    grad_norm = self.accelerator.clip_grad_norm_(
                        self.policy.parameters(), self.config.model.max_grad_norm
                    )  
                    # # grad_norm=self.policy.clip_grad_norm_(self.config.model.max_grad_norm)
                    # self.accelerator.print(f"grad_norm {grad_norm} \nself.config.model.max_grad_norm {self.config.model.max_grad_norm}")
                    # sys.exit()
...
class PairedPreferenceTrainer(BasicTrainer):
    ...
    def get_batch_metrics(
        ...
        metrics[f"rewards_{mode}/accuracies"] = self.accelerator.gather(
...
class RDPOTrainer(PairedPreferenceTrainer):
    def loss(
        ...
        *args,
        ...
        """Compute the RDPO loss for a batch of policy and reference model token-level log probabilities."""
        chosen_rewards = self.config.loss.beta * (
            policy_chosen_logps.sum(-1) - reference_chosen_logps.sum(-1)
        )
        rejected_rewards = self.config.loss.beta * (
        ...
        losses = -F.logsigmoid(
            chosen_rewards
            - rejected_rewards
            + alpha * (chosen_lengths - rejected_lengths)
        )

        f_loss = f_beta(chosen_rewards - rejected_rewards, beta=2)
        # ##fair reg
        # losses=losses-0.1*f_loss
        # ##fair coe
        losses = losses * (f_loss**0.5)
...
class CDPOTrainer(PairedPreferenceTrainer):
    def loss(
        ...
        *args,
        ...
        ) * forward_losses + self.config.loss.epsilon * reverse_losses
...
class SimPOTrainer(PairedPreferenceTrainer):
    def loss(
        ...
        *args,
        ...
        f_loss = f_beta(chosen_rewards - rejected_rewards, beta=0.5)
        ##fair reg
        # losses=losses-0.1*f_loss
        # ##fair coe
        losses = losses * (f_loss**0.5)
    ...
    def loss(
        ...
        *args,
        ...
        if policy_chosen_logps.shape[0] != 0:
            ...
            chosen_losses = (
                torch.Tensor([]).to(self.policy_dtype).to(self.accelerator.device)
            )
            chosen_rewards = (
                torch.Tensor([]).to(self.policy_dtype).to(self.accelerator.device)
            )
        ...
        if policy_rejected_logps.shape[0] != 0:
            ...
            rejected_losses = self.config.loss.undesirable_weight * (
                1 - F.sigmoid(self.config.loss.beta * (KL - rejected_rewards))
            )
        else:
            # important to cast to policy_dtype; otherwise error will occur during all_gather
            rejected_losses = (
        ...
        beta = 0.5
        if policy_chosen_logps.shape[0] != 0:
            f_loss += f_beta(chosen_rewards, beta=beta)
        # if policy_chosen_logps.shape[0] != 0:
        #     f_loss +=  f_beta(rejected_rewards,beta=beta)
        ###fair reg
        # losses+=-0.1*f_loss
        ###fair coe
        losses *= 1 + 0.01 * (f_loss + 1e-4) ** 0.5
        self.accelerator.print(f"f_loss {f_loss}\nlosses{losses}")
    ...
    def forward(
        ...
        with self.accelerator.autocast():
            with torch.no_grad():
                if use_cache:
                    KL_logps = (
                        model(batch[f"KL_combined_input_ids"])
                        .to(self.policy_dtype)
                        .to(self.accelerator.device)
                    )
                else:
            ...
            if use_cache:
                target_logps = (
                    model(batch[f"target_combined_input_ids"])
                    .to(self.policy_dtype)
                    .to(self.accelerator.device)
                )
            else:
                ...
                target_logps = self.get_batch_logps(
                    target_logits, batch[f"target_labels"]
                )

        assert target_logps.shape[0] == len(batch["status"])
        chosen_idx = [
            i for i in range(target_logps.shape[0]) if batch["status"][i] == "chosen"
        ]
        rejected_idx = [
            i for i in range(target_logps.shape[0]) if batch["status"][i] == "rejected"
        ]
        ...
        *args,
        ...
        if policy_rejected_logps.shape[0] != 0:
            ...
            rejected_losses = 1 - F.sigmoid(
                self.config.loss.beta * (0 - rejected_rewards)
            )
        else:
            # important to cast to policy_dtype; otherwise error will occur during all_gather
            rejected_losses = (
        ...
        losses = torch.cat(
            (
                self.config.loss.desirable_weight * chosen_losses,
                self.config.loss.undesirable_weight * rejected_losses,
            ),
            0,
        )
...
class PPOTrainer(BasicTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.reward_model is not None:
            ...
            self.reward_model, self.reward_tokenizer = self.accelerator.prepare(
                self.reward_model, self.reward_tokenizer
            )
    ...
    def forward(
        ...
        all_logps = self.get_batch_logps(
            all_logits.to(self.policy_dtype), batch["target_labels"]
        )
    ...
    def get_reward_scores(
        ...
        if self.reward_model is not None:
            ...
            sequences = self.tokenizer.batch_decode(
                batch["target_combined_input_ids"], skip_special_tokens=True
            )
            ...
            reward_scores = torch.tensor(
                [
                    (1 if batch["status"][i] == "chosen" else -1)
    ...
    def compute_advantages(
        ...
        for t in reversed(range(seq_len)):
            ...
            delta = (
                ...
                * (values[:, t + 1] if t < seq_len - 1 else 0.0)
                - values[:, t]
            )
            ...
            discounted_future_reward = (
                rewards[:, t] + self.config.loss.gamma * discounted_future_reward
            )
        ...
        discounted_future_rewards = (
            torch.stack(discounted_future_rewards_reversed[::-1]).transpose(0, 1)
            * masks
        ).contiguous()

        # normalizing advantages leads to more stable learning
        mean_adv, var_adv = masked_mean(advantages, masks), masked_var(
            advantages, masks
        )
    ...
    def loss(self, batch: Dict, episode: Dict) -> Tuple[torch.FloatTensor, Dict]:
        ...
        """
        value_losses = (
            episode["values"] - batch["discounted_future_rewards"].detach()
        ) ** 2
        ...
        policy_losses_clipped = -batch["advantages"] * torch.clamp(
            ratio, self.config.loss.cliprange, 1 / self.config.loss.cliprange
        )
        policy_loss = masked_mean(
            torch.max(policy_losses, policy_losses_clipped), batch["masks"]
        )

        KL_penalty = masked_mean(
            batch["logprobs"] - episode["logprobs"], batch["masks"]
        )

        loss = (
            policy_loss
            + self.config.loss.critic_coef * critic_loss
            + self.config.loss.KL_coef * KL_penalty
        )
        ...
        loss_stats = {
            ...
            "clipfrac/policy": masked_mean(
                torch.gt(policy_losses_clipped, policy_losses).float(), batch["masks"]
            ).detach(),
            "loss/entropy": entropy_from_logits(
                episode["logits"], batch["masks"]
            ).detach(),
            "loss/policykl": masked_mean(
                batch["logprobs"] - episode["logprobs"], batch["masks"]
            ).detach(),
    ...
    def get_global_batch_dict(self, batch):
        ...
        with torch.no_grad():
            masks = (
                (batch["target_labels"][:, 1:] != -100).clone().to(self.policy_dtype)
            )
            ...
            rewards = rewards * masks
            advantages, returns, discounted_future_rewards = self.compute_advantages(
        ...
        global_batch_dict = {
            k: v.to(self.accelerator.device) if isinstance(v, torch.Tensor) else v
            for k, v in global_batch_dict.items()
        }
    ...
    def train(self):
        ...
        for batch in self.train_iterator:
            ...
            for ppo_epoch in range(self.config.loss.ppo_epochs):
                with self.accelerator.accumulate(self.policy):
                    loss, local_batch_metrics = self.get_batch_metrics(
                        global_batch_dict, batch_size, mode="train"
                    )
                    ...
                    self.accelerator.backward(loss)
                    v_head_norm = self.accelerator.clip_grad_norm_(
                        self.policy.pretrained_model.parameters(),
                        self.config.model.max_grad_norm,
                    )
                    pretrained_norm = self.accelerator.clip_grad_norm_(
                        self.policy.v_head.parameters(),
                        self.config.model.v_head_max_grad_norm,
                    )
                    batch_metrics["grad_norm"].extend(
                        torch.as_tensor(v_head_norm + pretrained_norm)
                        .reshape(-1)
                        .float()
                        .cpu()
                        .numpy()
    ...
    def get_batch_metrics(
        self, global_batch_dict: Dict, batch_size: int = 0, mode: str = "train"
    ):
        """
        ...
        episode_logprobs, episode_logits, episode_values = self.forward(
            self.policy, shuffled_global_batch
        )
        episode = {
        ...
        metrics["returns/mean"] = masked_mean(
            shuffled_global_batch["returns"], shuffled_global_batch["masks"]
        ).detach()
        metrics["returns/var"] = masked_var(
            shuffled_global_batch["returns"], shuffled_global_batch["masks"]
        ).detach()
        metrics["val/mean"] = masked_mean(
            shuffled_global_batch["values"], shuffled_global_batch["masks"]
        ).detach()
        metrics["val/var"] = masked_var(
            shuffled_global_batch["values"], shuffled_global_batch["masks"]
        ).detach()
...
class BradleyTerryTrainer(PairedPreferenceTrainer):
    ...
    def get_batch_metrics(
        ...
        losses, chosen_rewards, rejected_rewards = self.loss(
            batch, policy_chosen_logits, policy_rejected_logits
        )
